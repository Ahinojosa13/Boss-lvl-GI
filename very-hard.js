...
//3
    this.fetchJob = function(){                                 
        console.log(this.name ,"is a", this.job); 	//Console logs property of name and job by calling it with this.(property)
    }
};

//4 By Uppercasing my P in Programmer its now a Constructor
function Programmer(name, job, age, languages) {        
...
// 6  
    this.offerNewTask = function(){                       
        if (this.busy){
            console.log(name,"can't accept any tasks right now")    
        }else{
...
// 7
    this.learnLanguage = function(newlanguages){
        languages.push(newlanguages)            // .push adds the new languages parameter to the languages property
    }
...
anthony.exercise()
anthony.fetchJob()
anthony.completeTask()
anthony.offerNewTask()
anthony.learnLanguage("C")
anthony.listLanguage()
